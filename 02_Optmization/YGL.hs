...
module YGL (
    ...
    , ConvexFunction3D -- Point -> Point -> Maybe (Point,Color)
    , Function3D -- Point -> Point -> Point -> Maybe Color
...
data Atom =   ColoredTriangle Triangle3D 
            | ColoredPoint (Point3D,Color)
            | ColoredQuad  (Point3D,Point3D,Point3D,Point3D,Color)

-- | A ConvexFunction3D is simply a function for each x,y associate a z and a color
-- | If undefined at point (x,y), it returns Nothing.
type ConvexFunction3D = Point -> Point -> Maybe (Point,Color)
type Function3D = Point -> Point -> Point -> Maybe Color

-- | Our objects that will be displayed
-- |    Wether a function3D delimited by a Box
-- |      or a list of Atoms
data YObject =   ConvexFunc ConvexFunction3D Box3D
               | Func Function3D Box3D
               | Atoms [Atom]

-- | The function atoms retrieve the list of atoms from an YObject
atoms :: YObject -> [Atom]
atoms (ConvexFunc f b) = atomsFromConvexFunction3D f b
atoms (Func f b) = atomsFromFunction3D f b
...
atomsFromFunction3D :: Function3D -> Box3D -> [Atom]
atomsFromFunction3D shape box = do
  x <- [xmin,xmin+res..xmax]
  y <- [ymin,ymin+res..ymax]
  z <- [zmin,zmin+res..zmax]
  let c = shape x y z
  if isNothing c 
     then []
     else cube (makePoint3D (x,y,z)) res (fromJust c)
  where
...
cube :: Point3D -> Scalar -> Color -> [Atom]
cube p res c =
  [ ColoredQuad (p0,p1,p2,p3,c)
  , ColoredQuad (p3,p2,p6,p7,c)
  , ColoredQuad (p7,p6,p5,p4,c)
  , ColoredQuad (p4,p5,p1,p0,c)
  , ColoredQuad (p5,p6,p2,p1,c)
  , ColoredQuad (p7,p4,p0,p3,c)
  ]
  where
    d=res/2
    p7=p + makePoint3D ( d, d, d)
    p6=p + makePoint3D ( d, d,-d)
    p5=p + makePoint3D ( d,-d,-d)
    p4=p + makePoint3D ( d,-d, d)
    p3=p + makePoint3D (-d, d, d)
    p2=p + makePoint3D (-d, d,-d)
    p1=p + makePoint3D (-d,-d,-d)
    p0=p + makePoint3D (-d,-d, d)

-- | Given a shape function and a delimited Box3D
-- | return a list of Atoms (here only colored triangles) to be displayed
atomsFromConvexFunction3D :: ConvexFunction3D -> Box3D -> [Atom]
atomsFromConvexFunction3D shape box = do
  x <- [xmin,xmin+res..xmax]
  y <- [ymin,ymin+res..ymax]
  let 
...
drawObject shape = mapM_ drawAtom (atoms shape)
...
drawAtom (ColoredPoint (p,c)) = do
    renderPrimitive Points $ do
        color c
        vertex $ toGLVertex3 p
drawAtom atom@(ColoredQuad (p1,p2,p3,p4,c)) = do
    renderPrimitive Quads $ do
        color c
        normal $ toGLNormal3 (getNormal atom)
        vertex $ toGLVertex3 p1
        vertex $ toGLVertex3 p2
        vertex $ toGLVertex3 p3
        vertex $ toGLVertex3 p4
...
getNormal (ColoredQuad (p0,p1,p2,_,_)) = (p1 - p0) * (p2 - p0)
